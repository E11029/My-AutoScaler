...
#define MAX_SLOTS 1	//Total number of slots 352
...
#define TOTAL_PATHS 4		//Total paths we want to get (in allpathsmap) Always>=MAX_TRIAL_PATHS
#define MAX_TRIAL_PATHS 4			//Changes when graph changes (Total posible paths for primary and backup)
//=======================CHANGES WHEN GRAPH CHANGES================================

//=======================CHANGES WHEN LAYERS CHANGE================================
#define MAX_LAYERS 500			//Maximum size of layers
#define LAYERID 10			//Start value of Layer id
//=======================CHANGES WHEN LAYERS CHANGE================================

#define SHLPIDSTART 1001		//Initial Shared ID of a shared_id map
...
#define LOAD50 50		//Load. and it varies.
#define LOAD80 80		//Load. and it varies.
#define LOAD100 100		//Load. and it varies.
#define LOAD120 120		//Load. and it varies.
#define LOAD150 150		//Load. and it varies.
#define LOAD200 200		//Load. and it varies.
#define LOAD300 300		//Load. and it varies.
#define LOAD500 500		//Load. and it varies.
//=========================SIMULATION SETUP=====================================
...
int findpath(int s,int d,int path[max_nodes+1],int *sdist);//, int n1, int n2);
void print_shortest_links(int source, int dest);//, int node1, int node2);
...
IntVec PrimSlots;		//For storing primary slots
...
LOADVec.push_back(LOAD80); 
LOADVec.push_back(LOAD100); 
//LOADVec.push_back(LOAD120);
//LOADVec.push_back(LOAD150);
//LOADVec.push_back(LOAD200);
//LOADVec.push_back(LOAD300);
//LOADVec.push_back(LOAD500);
...
//------------------------------------------------------------------------------------------------------------------
//FIND SHORTEST PATH OF A GIVEN GRAPH
int findpath(int s,int d,int path[max_nodes+1],int *sdist)	//, int n1, int n2)
{
	struct node state[max_nodes+1];
	...
	*sdist=0;
	...
	while(current!=d)
	{
		for(i=1;i<=max_nodes;i++)
		{
			...
			if ( adj[current][i] > 0 && state[i].status == TEMP )	//&&((current!=n1)&&(i!=n2)||(current!=n2)&&(i!=n1))
			{
		...
		for(i=1;i<=max_nodes;i++)
		{
			if(state[i].status == TEMP && state[i].dist < min)		//&&((current!=n1)&&(i!=n2)||(current!=n2)&&(i!=n1))
			{
				min = state[i].dist;
				current=i;
...
//------------------------------------------------------------------------------------------------------------------
//PRINT LINKS ID AND DISTANCE OF SHORTEST PATH
void print_shortest_links(int source, int dest)//, int node1, int node2)
{
	ppnodes.clear();
	pplinks.clear();
	...
	count = findpath(source,dest,path,&shortdist);//, node1, node2);
	//cout<<"Total nodes on shortest path: "<<count<<"\n";
	//cout<<"\nSHELTON"<<shortdist;
	...
	if(shortdist!=0)
	{
		...
		for(i=count;i>1;i--)
		{
			...
			if(i==count)
			{
				ppnodes.push_back(path[i]);				//For storing paths (1->3, 3->6)
				ppnodes.push_back(path[i-1]);
			}			
			else
			{
				ppnodes.push_back(path[i-1]);
			...
			for(itg=graphtextmap.begin(), fit=fiber_link.begin(); itg!=graphtextmap.end(), fit!=fiber_link.end(); ++itg, ++fit)
			{
				for(size_t n=0; n<(*itg).second.size(); ++n)
				{
					if((itg->second[0]==path[i])&&(itg->second[1]==path[i-1]))
					{						
						pplinks.push_back(itg->first);
						comLinks.push_back(fit->second);
						cout<<itg->first<<"  "<<fit->second;
						break;
					}
				}
			}
		...
		cout <<"Primary Nodes Are : ";
		for (IntVec::iterator itnode=ppnodes.begin(); itnode<ppnodes.end(); itnode++)
		{
			cout<<(*itnode)<<" ";
		}
		cout<<"\n";

		cout <<"Primary Links Are : ";
		for (itppl=pplinks.begin(); itppl<pplinks.end(); itppl++)
		{
			cout<<(*itppl)<<" ";
		}
		cout<<"\n";

		cout <<"Combined Links Are : ";
		for (IntVec::iterator clink=comLinks.begin(); clink<comLinks.end(); clink++)
		{
			cout<<(*clink)<<" ";
		}
		cout<<"\n";
		//primarypath[Lightpath] = pplinks;								//Inserting lightpath, links into primarypath map		
	}
	else
	{
		cout<<"\n\nTHERE IS NO PATH FROM SOURCE TO DESTINATION (BLOCKED DUE TO NO PRIMARY PATH)\n";
	}
	return;
}
//------------------------------------------------------------------------------------------------------------------
...
IntVec select_slots_for_primarypath()
{
	cout<<"\n\nPRIMARY";
	cout<<"\n=========";
	cout<<"\nStart...End";
	...
	for (int startIndex = 0; startIndex < MAX_SLOTS + 1 - No_Of_Slots; startIndex++) 
	{
		...
		{      
			...
			PrimSlots.push_back(startIndex);
			PrimSlots.push_back(startIndex + No_Of_Slots - 1);
...
TEMPY.clear();
PrimSlots.clear();
while (PrimSlots.empty())
{
	//for(size_t i=0; i<TEMPY.size(); i++)
	//{
		print_shortest_links(start, end);	//, n, m);

		{
		PrimSlots = select_slots_for_primarypath();
		}
	//}
}
...
for(IntVec::iterator ite = PrimSlots.begin(); ite != PrimSlots.end(); ++ite)
{
	cout<<(*ite)<<" ";
}
...
bandblock=((double(count (RATESBLOCKED.begin(), RATESBLOCKED.end(), 10))*10)+(double(count (RATESBLOCKED.begin(), RATESBLOCKED.end(), 40))*40)+(double(count (RATESBLOCKED.begin(), RATESBLOCKED.end(), 100))*100))/(double(RATETOTAL[0]*10)+double(RATETOTAL[1]*40)+double(RATETOTAL[2]*100));
