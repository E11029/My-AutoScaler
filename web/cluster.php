...
$query = mysql_query("insert into cluster (clustername,sub_name,Num_servers,Num_servers_base,max_servers,cpu_W,cpu_C ,cpu_F,Mem_W,Mem_C,Mem_F,max_server,monitor,Created_date) 
VALUES($clustername,$sub_name,$Num_servers_base,$Num_servers_base,$max_servers,$cpu_W,$cpu_C ,$cpu_F,$Mem_W,$Mem_C,$Mem_F,$max_server,$monitor,now());", $connection);
$rows = mysql_num_rows($query);
$query = mysql_query("select clustername from cluster where clustername==$clustername;", $connection);
$rows = mysql_num_rows($query);
if ($rows == 1) {
...
<form style="margin-top:5%;" action="">
  ...
  <label class="form-control-label" for="inputSuccess1">Cluster Name</label>
	...
	<div class="checkbox">
      <label>
        <input type="checkbox" name="monitor"> Enable monitoring
      </label>
    </div>
